...
bool gt1x_dev_suspended = false;
#if GTP_CHARGER_SWITCH
static bool charging_status = false;
static bool charging_status_change = false;
static DEFINE_MUTEX(mutex_charging);
#endif
...
**/
#if GTP_TOUCH_MODE
static u8 gt1x_touch_mode = 0;
static u8 gt1x_touch_mode_change = false;
static DEFINE_MUTEX(mutex_cover);

static int gt1x_cover_select(int cover_type)
{
	s32 ret = 0;
	...
	if ((cover_type != 0) && (cover_type != 1) && (cover_type != 2)) {
	...
	GTP_DEBUG("success to set touch mode as %d, sensor id = %d",
			gt1x_touch_mode, gt1x_version.sensor_id);
	gt1x_touch_mode_change = false;
	return 0;

error:
	GTP_ERROR("gt1x_cover_select fail");
	return -1;
}

void gt1x_change_touchmode(void)
{
    mutex_lock(&mutex_cover);
    if(gt1x_touch_mode_change)
    {
        gt1x_cover_select(gt1x_touch_mode);
    }
    mutex_unlock(&mutex_cover);
    GTP_DEBUG("change touchmode  done!");
}
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	mutex_lock(&mutex_cover);
	if(gt1x_dev_suspended == true)
	{
		gt1x_touch_mode = recv;
		gt1x_touch_mode_change = true;
		mutex_unlock(&mutex_cover);
		GTP_INFO("suspend statue to set touch mode as %d",gt1x_touch_mode);
		return size;
	}

	if(0 > gt1x_cover_select(recv))
	{
		GTP_ERROR("gt1x_cover_select fail");
	}
	mutex_unlock(&mutex_cover);
...
void gt1x_charger_change_config(bool charging_status_ischange)
{
    mutex_lock(&mutex_charging);
    if(!charging_status_ischange)
    {
        mutex_unlock(&mutex_charging);
        return;
    }
    gt1x_charger_config(charging_status);
    mutex_unlock(&mutex_charging);
}
 ...
 *
 */
void gt1x_charger_config(bool charging_status)
{
#if GTP_TOUCH_MODE
    ...
    if (charging_status) {
            if (gt1x_send_cmd(GTP_CMD_CHARGER_ON, 0)) {
                GTP_ERROR("Update cmd for Charger Plugin failed!");
            }
    } else {
            if (gt1x_send_cmd(GTP_CMD_CHARGER_OFF, 0)) {
               GTP_ERROR("Update cmd for Charger Plugout failed!");
            }
    }
	charging_status_change = false;
}

int gt1_power_notifier_cb(struct notifier_block *self,
		unsigned long val, void *v)
{
	...
	if (IS_ERR(psy)) {
		GTP_ERROR("psy  is  NULL \n");
		return NOTIFY_OK;
	}
	if (IS_ERR(psy->get_property) || psy->get_property == NULL) {
		GTP_ERROR("psy  get_property  is  NULL \n");
		return NOTIFY_OK;
	}
	...
	if (status != POWER_SUPPLY_STATUS_DISCHARGING && last_status == POWER_SUPPLY_STATUS_DISCHARGING) {
		...
		if(!gt1x_dev_suspended)
			gt1x_charger_config(charging_status);
		else
			charging_status_change = true;
		mutex_unlock(&mutex_charging);
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	GTP_DEBUG("success to set palm sleep as %d", gt1x_palm_sleep);
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	GTP_DEBUG("success to set wakeup gesture as %d", gt1x_wakeup_gesture);
...
int gt1x_suspend(void)
{
    ...
    if(gt1_pinctrl_set_state(false)){
        GTP_ERROR("pinctrl set suspend failed.\n");
}
#endif
...
int gt1x_resume(void)
{
    ...
    {
        gt1x_dev_suspended = false;
#if GTP_TOUCH_MODE
        gt1x_change_touchmode();
#endif
#if GTP_CHARGER_SWITCH
        gt1x_charger_change_config(charging_status_change);
#endif
        return 0;
    }
