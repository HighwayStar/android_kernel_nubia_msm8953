...
static struct reg_config mxt_dwakeup_cfg[] = {
	...
	{.reg = MXT_SPT_SELFCAPCONFIG_T111, .instance = 0,
		.offset = 0,.buf = {128}, .len = 1, .mask = 0,.flag = BIT_MASK(OP_SET)},

	{.reg = MXT_GEN_ACQUISITIONCONFIG_T8, .instance = 0, 
		.offset = 10,.buf = {3, 2, 2}, .len = 3, .mask = 0,.flag = BIT_MASK(OP_SET)},

	{.reg = MXT_SPT_GPIOPWM_T19, .instance = 0, 
		.offset = 0,.buf = {1,0,0,63}, .len = 4, .mask = 0,.flag = BIT_MASK(OP_SET)},

	{.reg = MXT_SPT_CTECONFIG_T46,
		.offset = 2,.buf = {6}, .len = 1, .mask = 0,.flag = BIT_MASK(OP_SET)},
	...
	{.reg = MXT_TOUCH_MULTITOUCHSCREEN_T100,
		.offset = 32,.buf = {35}, .len = 1, .mask = 0,.flag = BIT_MASK(OP_SET)},
	...
	{.reg = MXT_GEN_POWERCONFIG_T7,
		.offset = 0,.buf = {70,16, 5, 64}, .len = 4, .mask = 0,.flag = BIT_MASK(OP_SET)},


	{.reg = MXT_PROCI_AUXTOUCHCONFIG_T104,
		.offset = 2,.buf = {40}, .len = 1, .mask = 0,.flag = BIT_MASK(OP_SET)},

	{.reg = MXT_PROCI_AUXTOUCHCONFIG_T104,
		.offset = 7,.buf = {40}, .len = 1, .mask = 0,.flag = BIT_MASK(OP_SET)},
/*
...
static int init_pi_object(struct plugin_proci *p)
{
	...
	dev_info(dev, "%s: alloc mem %d, each %ld\n", 
