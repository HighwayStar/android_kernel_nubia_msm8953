...
//nubia for synaptics pinctrl
	...
	INIT_WORK(&rmi4_data->resume_work, synaptics_rmi4_resume_work);
#endif
...
static void synaptics_rmi4_resume_work(struct work_struct *work)
{
	struct synaptics_rmi4_data *rmi4_data =container_of(work, struct synaptics_rmi4_data, resume_work);
...
static int synaptics_rmi4_resume(struct device *dev)
{
	...
	if(rmi4_data->suspend) {
		...
		synaptics_rmi4_sleep_enable(rmi4_data, false);
		ret = synaptics_rmi4_irq_enable(rmi4_data, true, false);
		if (ret < 0) {
			dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to enable irq\n",
				__func__);
		}
