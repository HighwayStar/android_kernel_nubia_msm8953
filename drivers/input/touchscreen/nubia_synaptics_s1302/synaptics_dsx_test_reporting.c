...
#define SYNAPTICS_FORMAT_PATH(path,mdir,name) do{\
                               struct timex txc;\
                               struct rtc_time tm;\
...
static int synaptics_write_file(char *file_path, const char *write_buf, int count)
{
	struct file *file_p;
	mm_segment_t old_fs;
		...
		struct device_attribute *attr, char *buf)
{
	...
	SYNAPTICS_FORMAT_PATH(file_save_path,TPLOG_DIR,"key_rawdata");
	synaptics_write_file(file_save_path,buf,count);
	return count;
}
