...
static int parse_dt(struct device *dev, struct synaptics_dsx_board_data *bdata)
{
	...
	pr_debug("[TP]%s: synaptics irq-gpio = %d\n", __func__, bdata->irq_gpio);
	...
	pr_debug("[TP]%s: synaptics1302 irq-on-state = %d\n", __func__, bdata->irq_on_state);
	...
	{
		bdata->reset_gpio = -1;
		pr_debug("[TP]%s: can't get synaptics1302 reset gpio !\n", __func__);
	}
	pr_debug("[TP]%s: synaptics reset-gpio = %d\n", __func__, bdata->reset_gpio);

	retval = of_property_read_u32(np, "synaptics,reset-on-state",&value);
	if (retval < 0){
		bdata->reset_on_state = -1;
		pr_debug("[TP]%s: can't get synaptics1302 reset-on-state !\n", __func__);
	}
	else
	{
		bdata->reset_on_state = value;
	}
	pr_debug("[TP]%s: synaptics1302 reset-on-state = %d\n", __func__, bdata->reset_on_state);

	retval = of_property_read_u32(np, "synaptics,reset-active-ms",&value);
	if (retval < 0)
	{
		bdata->reset_active_ms = 0;
		pr_debug("[TP]%s: can't get synaptics1302 reset-active-ms !\n", __func__);
	}
	else
	{
		bdata->reset_active_ms = value;
	}
	pr_debug("[TP]%s: synaptics1302 reset-active-ms = %d\n", __func__, bdata->reset_active_ms);

	retval = of_property_read_u32(np, "synaptics,reset-delay-ms",&value);
	if (retval < 0)
	{
		bdata->reset_delay_ms = 0;
		pr_debug("[TP]%s: can't get synaptics1302 reset_delay_ms !\n", __func__);
	}
	else
	{
		bdata->reset_delay_ms = value;
	}
	pr_debug("[TP]%s: synaptics1302 reset_delay_ms = %d\n", __func__, bdata->reset_delay_ms);
	...
	if (retval < 0)
	{
		...
		pr_debug("[TP]%s: synaptics1302 can't get max_y_for_2d !\n", __func__);
	}
	else
	{
	...
	if (bdata->lcd_reset_gpio < 0)
	{
		...
		pr_debug("[TP]%s: can't get synaptics1302 lcd reset gpio !\n", __func__);
	}
	pr_debug("[TP]%s: synaptics1302 lcd-reset-gpio = %d\n", __func__, bdata->lcd_reset_gpio);
	...
	{
		bdata->ub_i2c_addr = -1;
		pr_debug("[TP]%s: synaptics1302 can't get ub_i2c_addr !\n", __func__);
	}
	else
	{
