...
extern bool weak_charger_status(void);
extern int get_prop_charger_voltage(void);
//extern int get_prop_pmic_temp(void);
extern void dump_chg_status_reg(void);
...
static void nubia_batt_worker(struct work_struct *work)
{
	...
	if(pbatt->usb_in){
		 dump_chg_status_reg();
		 pr_err("batt_soc=%d, batt_ma=%d, batt_mv=%d,batt_temp=%d, batt_st=%d, usb_in=%d, wk_chg=%d, vbus=%d \n",
		      pbatt->batt_soc,pbatt->batt_ma,pbatt->batt_mv, pbatt->batt_temp,pbatt->batt_status,pbatt->usb_in, weak_charger_status(),get_prop_charger_voltage());
	}

	if(power_supply_change){
		NXFG_DEBUG("battery status changed.\n" );
