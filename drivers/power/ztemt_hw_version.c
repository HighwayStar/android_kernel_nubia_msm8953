...
//开始进入第一大部分-读取pcb版本部分
#ifdef CONFIG_ZTEMT_HW_VER_BY_ADC
const struct hw_pcb_adc_map_str *ztemt_get_pcb_table_item_by_adc(
									const struct hw_pcb_adc_map_str *pts,
									uint32_t tablesize,
									int input
									)
{
...
#else
const struct hw_pcb_gpio_map_str *ztemt_get_pcb_table_item_by_gpio(
									const struct hw_pcb_gpio_map_str *pts,
									uint32_t tablesize,
									char *gpio_str
									)
{
...
//开始读取第三大部分-软件相关的一些配置
#ifdef CONFIG_ZTEMT_HW_CONFIG_BY_GPIO
const struct hw_config_gpio_map_st *ztemt_get_config_table_item_by_gpio(
										const struct hw_config_gpio_map_st *pts,
										uint32_t tablesize,
										char *gpio_str
										)
{
...
#else
const struct hw_config_adc_map_st *ztemt_get_config_table_item_by_adc(
										const struct hw_config_adc_map_st *pts,
										uint32_t tablesize,
										int input
										)
{
...
//读取天线配置
#ifdef CONFIG_ZTEMT_ANT_VER_BY_GPIO
const struct hw_ant_gpio_map_str *ztemt_get_ant_table_item_by_gpio(const struct
								   hw_ant_gpio_map_str
								   *pts,
								   uint32_t
								   tablesize,
...
static void ztemt_cpu_serial_num_init(void)
{
	...
	//MSM8976  0x000A60A8
	//MSM8953  0x000A4128
#define QFPROM_CPU_SERIAL_NUM_LSB  0x000A4128
#define QFPROM_CPU_SERIAL_NUM_MSB  0
	...
	if (!serial_num_addr) {
		printk(KERN_ERR
		       "ztemt could not map QFPROM_CPU_SERIAL_NUM_LSB address\n");
		g_cpu_serial_num = 0;
	} else {
		g_cpu_serial_num = readl_relaxed(serial_num_addr);
		printk(KERN_ERR "ztemt cpu_serial_num=0x%08X\n",
		       g_cpu_serial_num);
...
int __init ztemt_hw_version_init(void)
{
	...
	if (rc) {
		printk(KERN_ERR
		       "%s: failed to create ztemt_hw_version group attributes\n",
		       __func__);
	}
